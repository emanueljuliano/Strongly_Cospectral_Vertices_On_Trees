...
def trees(order):
        ...
        layout = [i for i in range(order // 2 + 1)] + [i for i in range(1, (order + 1) // 2)]
...
def layout_to_matrix(layout):
        ...
        print("Idepotents: ")
        for v in np.linalg.eig(T)[1]:
            print(np.matrix(v).T@np.matrix(v))
        ''' 
        return result
...
if __name__ == "__main__":
        ...
        print("Number of trees ", functools.reduce(lambda accum, tree: accum + 1,
                generator, 0))
